				  ...
				  unsigned int nr_blockgroups)
{
	...
	for (i = 0; i < nr_blockgroups; i++, bgs++, blockgroups++) {
		/*
		 * Only changes to fs_flags and the writepointer are expected.
		 * But we update almost everything here for consistency except
		 * for the zone mapping, as it points to our internal
		 * copy of struct blk_zone *zone.
		 */
		...
		blockgroups->fs_flags = ntohl(bgs->fs_flags);
	}
