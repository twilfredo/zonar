...
#ifdef XFS_RG_WPTR
static int znr_xfs_rg_get_wptr(unsigned int rgno, unsigned long long *wptr)
{
				   ...
				   unsigned int *nr_blockgroups)
{
	...
	for (rg = 0; rg < rgcount && idx < max_blockgroups; rg++, idx++) {
		 ...
		 * success.
		 */
		ret = znr_xfs_rg_get_wptr(rg, &wptr);
		if (ret < 0 && ret != -ENOTSUP)
			return ret;
		if (!ret) {
			bgs[idx].fs_has_wp = true;
			bgs[idx].wp_sector = wptr;
		}
	}
				      ...
				      unsigned int nr_blockgroups)
{
	...
	for (unsigned int i = 0; i < nr_blockgroups; i++) {
		...
		ret = znr_xfs_rg_get_wptr(rgno, &wptr);
		if (ret < 0 && ret != -ENOTSUP)
			return ret;
		if (!ret) {
			blockgroups[i].fs_has_wp = true;
			blockgroups[i].wp_sector = wptr;
		}
	}
