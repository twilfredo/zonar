...
#ifdef XFS_HAS_RG_WP
static int znr_xfs_get_rg_fs_write_pointer(unsigned int rgno, struct znr_bg *bg)
{
	...
	bg->fs_wp_sector = rt_geom->rg_writepointer * (off_t)fs_geo.blocksize /
			BBSIZE;
	bg->flags |= ZNR_BG_HAS_FS_WP;

	if (bg->fs_wp_sector >= bg->nr_sectors)
				   ...
				   unsigned int *nr_blockgroups)
{
	...
	for (rg = 0; rg < rgcount && idx < max_blockgroups; rg++, idx++) {
		 ...
		 * success.
		 */
		ret = znr_xfs_get_rg_fs_write_pointer(rg, &bgs[idx]);
		if (ret == -ENOTSUP)
			continue;
				      ...
				      unsigned int nr_blockgroups)
{
	...
	for (unsigned int i = 0; i < nr_blockgroups; i++) {
		/* For AGs, do nothing */
		if (blockgroups[i].sector < rtstart)
			continue;

		rgno = (blockgroups[i].sector - rtstart) / bbperrg;
		ret = znr_xfs_get_rg_fs_write_pointer(rgno, &blockgroups[i]);
		if (ret == -ENOTSUP)
			continue;
