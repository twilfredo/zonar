					     ...
					     unsigned long max_nr_sectors)
{
	...
	w = max_width * write_pointer / max_nr_sectors;
	if (w > max_width)
		return max_width;
	return w;
}
					    ...
					    int width, int height)
{
	...
	dw = znr_gui_get_wp_width(width, wp_delta, bg->nr_sectors);
	if (dw == width)
		dw = width - w;
	if (!dw)
		return;
				       ...
				       gpointer user_data)
{
	...
	if (blockgroup && blockgroup->hovered) {
		...
		if (znr_bg_has_wp(bg)) {
			...
			if (bg->flags & ZNR_BG_HAS_DEV_ZONE_WP) {
				wp_sector = bg->dev_zone_wp_sector;
				bg_full = znr_bg_dev_zone_full(bg);
			} else {
				wp_sector = bg->fs_wp_sector;
				bg_full = znr_bg_full(bg);
			}
					...
					gpointer user_data)
{
	...
	bg = blockgroup->bg;
	ret = znr_fs_get_extents_in_range(bg->sector,
					  bg->nr_sectors,
					  &extents, &nr_extents);
	if (ret) {
