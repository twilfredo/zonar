				   ...
				   unsigned int zone_sectors)
{
	...
	for (i = 0; i < nr_blockgroups; ++i) {
		...
		if (!(blockgroups[i].flags & ZNR_BG_HAS_FS_WP) &&
		    blockgroups[i].zones[0]->type == BLK_ZONE_TYPE_SEQWRITE_REQ) {
			if (blockgroups[i].zones[0]->cond == BLK_ZONE_COND_FULL)
				blockgroups[i].flags |= ZNR_BG_FULL;

			blockgroups[i].wp_sector =
				blockgroups[i].zones[0]->wp -
				blockgroups[i].sector;

			blockgroups[i].flags |= ZNR_BG_HAS_ZONE_WP;
		}
	}
