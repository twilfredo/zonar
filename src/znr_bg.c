...
static inline void znr_bg_get_zone_wp(struct znr_bg *blockgroup)
{
	...
	if (!(blockgroup->flags & ZNR_BG_HAS_FS_WP) &&
	    blockgroup->zones[0]->type == BLK_ZONE_TYPE_SEQWRITE_REQ) {
		if (blockgroup->zones[0]->cond == BLK_ZONE_COND_FULL)
			blockgroup->flags |= ZNR_BG_FULL;

		blockgroup->wp_sector = blockgroup->zones[0]->wp -
					blockgroup->sector;
		blockgroup->flags |= ZNR_BG_HAS_ZONE_WP;
	}
}
				   ...
				   unsigned int zone_sectors)
{
	...
	znr_bg_get_zone_wp(blockgroup);
	blockgroup->nr_zones = bg_zone_idx;
	blockgroup->flags |= ZNR_BG_HAS_ZONE_INFO;
	*zone_idx = j;
	return ret;
}
