...
void znr_bgs_destroy(struct znr_blockgroup *bgs, unsigned int nr_bgs)
{
	unsigned int i;
				   ...
				   unsigned int zone_sectors)
{
	...
	for (i = 0; i < nr_bgs; ++i) {
		...
		bgs[i].flags = bgs[i].zones[0]->type;
		if (bgs[i].flags == BLK_ZONE_TYPE_SEQWRITE_REQ)
			bgs[i].wp_sector =
				bgs[i].zones[0]->wp -
				bgs[i].sector;
		else
			 ...
			 unsigned int nr_bgs)
{
	...
	if (!dev->is_zoned) {
		 ...
		 */
		for (i = 0; i < nr_bgs; i++)
			bgs[i].flags = BLK_ZONE_TYPE_CONVENTIONAL;
		return nr_bgs;
	}
